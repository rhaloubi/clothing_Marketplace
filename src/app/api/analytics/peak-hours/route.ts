...
import { fetchAnalyticsPeakHoursSnapshot } from "@/lib/server/analytics-modules"
...
export const GET = withUserAuth(
  withPlan("has_analytics")(
    withRateLimit("api", { keyBy: "user" })(async (req: NextRequest, { auth }) => {
      ...
      const snapshot = await fetchAnalyticsPeakHoursSnapshot(supabase, range.store_id, range.window)
      return ok(snapshot)
    })
  )
)
